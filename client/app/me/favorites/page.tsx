...
export default function FavoritesPage() {
  ...
  const [filteredEpisodes, setFilteredEpisodes] = useState<WPEpisode[]>([]);
  const [hasListRows, setHasListRows] = useState(false);
  ...
  useEffect(() => {
    ...
    const loadFavorites = async () => {
      setIsLoading(true);
      try {
        const listRows = await meApi.getLists({ list_type: 'favorite' });
        if (!isMounted) return;

        if (listRows.length === 0) {
          setHasListRows(false);
          setMovieCards([]);
          setSeriesCards([]);
          setFilteredEpisodes([]);
          return;
        ...
        setHasListRows(true);
        const { filteredMovies, filteredSeries, filteredEpisodes: episodes } =
          await fetchListItems(listRows);
        if (!isMounted) return;

        const [moviePosters, seriesPosters] = await Promise.all([
          Promise.all(filteredMovies.map(m => getPosterUrl(m.featured_media))),
          Promise.all(filteredSeries.map(s => getPosterUrl(s.featured_media))),
        ]);
        if (!isMounted) return;

        const nextMovieCards: MovieCardData[] = filteredMovies.map((m, i) => ({
          id: m.id,
          slug: m.slug,
        ...
        const nextSeriesCards: SeriesCardData[] = filteredSeries.map((s, i) => ({
          id: s.id,
          slug: s.slug,
        ...
        setMovieCards(nextMovieCards);
        setSeriesCards(nextSeriesCards);
  ...
  if (movieCards.length === 0 && seriesCards.length === 0 && filteredEpisodes.length === 0) {
    return (
      <PageContainer title="Favorites" description="Your favorite titles">
        <div className="rounded-2xl border border-black/10 bg-white p-8 text-center dark:border-white/10 dark:bg-slate-800">
          <div className="text-sm font-semibold text-slate-900 dark:text-slate-100">
            {hasListRows ? 'No items found' : 'Your favorites list is empty'}
          </div>
