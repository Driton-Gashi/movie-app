...
export default function Sidebar({ open, onClose }: SidebarProps) {
  ...
  const SidebarInner = (
    <>
      ...
      <div className="flex h-16 items-center gap-2 border-b border-black/10 px-4">
        <Link href="/" className="flex items-center gap-2" aria-label="Go to homepage">
          ...
          <div className="leading-tight">
            <div className="text-sm font-semibold text-slate-900">MovieApp</div>
      ...
      <div className="border-t border-black/10 p-3">
        <Link
          href="/settings"
          className="block rounded-lg px-3 py-2 text-sm font-medium text-slate-700 hover:bg-slate-100 hover:text-slate-900"
        >
          Settings
        </Link>
      </div>
  ...
  return (
    <>
      ...
      <aside className="hidden md:sticky md:top-0 md:z-30 md:flex md:h-screen md:w-64 md:flex-col md:border-r md:border-black/10 md:bg-white">
        {SidebarInner}
      </aside>
      ...
      <div className={cn('md:hidden', open ? 'block' : 'hidden')}>
        ...
        />
        {/* Panel */}
        <aside className="fixed inset-y-0 left-0 z-50 w-80 max-w-[85vw] border-r border-black/10 bg-white">
          <div className="flex h-16 items-center justify-between border-b border-black/10 px-4">
            ...
            <button
              type="button"
              onClick={onClose}
              className="rounded-lg p-2 text-slate-700 hover:bg-slate-100 focus:outline-none focus:ring-2 focus:ring-slate-900"
              aria-label="Close"
            >
              ✕
            </button>
